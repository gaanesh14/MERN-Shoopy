  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        user.response?.data || { message: "Login failed" },
      );
    }
  },
);
...
export const registerUser = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        user.response?.data || { message: "Registration failed" },
      );
    }
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(loginUser.pending, (state) => {
        state.loading = true;
        ((state.loginStatus = "loading"), (state.user = null));
      })
      .addCase(loginUser.fulfilled, (state, action) => {
        state.loading = false;
        ((state.loginStatus = "success"), (state.user = action.payload));
        // state.user = null;
      })
